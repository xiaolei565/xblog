...
`Explain` 执行计划包含字段信息如下：分别是 `id`、`select_type`、`table`、`partitions`、`type`、`possible_keys`、`key`、`key_len`、`ref`、`rows`、`filtered`、`Extra` 12 个字段。![](https://imgconvert.csdnimg.cn/aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy8wT3phTDV1VzJhT0QzaWM2MGhMQ1pVT2JLMUpXTnB1aE5tSjRnck5pY2x2ZEVYYVJVVGI5S0tscVo0WVc2R09XTzdKdWx4Nko3Z1BDajFrVlJpYlFXM1VVQS82NDA?x-oss-process=image/format,png)下边我们会结合具体的`SQL`示例，详细的解读每个字段以及每个字段中不同参数的含义，以下所有示例数据库版本为 `MySQL.5.7.17`。
...
`Extra`列的信息非常非常多，这里就不再一一列举了，详见 `MySQL`官方文档 ：https://dev.mysql.com/doc/refman/5.7/en/explain-output.html#jointype_index_merge![](https://imgconvert.csdnimg.cn/aHR0cHM6Ly9tbWJpei5xcGljLmNuL21tYml6X3BuZy8wT3phTDV1VzJhT0QzaWM2MGhMQ1pVT2JLMUpXTnB1aE42aWNtNEg2UUVpYnlpYnA3bDJ2Y1prZ3pJTTBxZzRBcTF2eTFJSDFCN3FveThSUTkzS21tY2djaWJnLzY0MA?x-oss-process=image/format,png)
...
[Explain执行计划中各个字段的含义](https://blog.csdn.net/u014532775/article/details/106623994)
